...
class AgcViewer extends HTMLElement {
  ...
  _initViewer() {
    ...
    this._camera = new THREE.PerspectiveCamera(48, width / height, 0.01, 1000);
  ...
  async _loadScene() {
    ...
    if (!globalDecoder) {
      this.dispatchEvent(new CustomEvent('error', {
        detail: new Error('Load agc_decoder.js before agc_viewer.js.'),
      }));
      return;
    }
      ...
      if (typeof Worker !== 'undefined') {
        try {
          decoded = await new Promise((resolve, reject) => {
            ...
            // Clone, do not transfer: keep `buffer` for main-thread fallback.
  ...
  _buildSplatMesh(decoded) {
    ...
    const instancedMesh = new THREE.InstancedMesh(
      ...
      new THREE.ShaderMaterial({
        ...
        uniforms: {
          uViewport: this._matUniforms.uViewport,
          uFocal: this._matUniforms.uFocal,
        },
        vertexShader: `
          ...
          void main() {
            ...
            float invz = 1.0 / max(-pCam.z, 0.05);
            float sx = uFocal.x * invz;
            float tr = max(cov0.x + cov1.x + cov1.z, 1.0e-10);
            float rms = sqrt(tr);
            if (rms > 0.35) {
              gl_Position = vec4(0.0, 0.0, 2.0, 1.0);
              return;
            }
            float radPx = clamp(rms * sx * 1.4, 0.9, 12.0);


            vec4 clip = projectionMatrix * pCam;
            clip.xy += corner * radPx * 2.0 / max(uViewport, vec2(1.0)) * clip.w;
            gl_Position = clip;
          }
        `,
        fragmentShader: `
          ...
          void main() {
            ...
            if (r2 > 1.0) discard;
            float alpha = vColor.a * exp(-2.0 * r2);
    ...
    if (this.getAttribute('up') === 'z') {
      ...
      this._controls.target.set(-0.2, 0.0, 0.0);
