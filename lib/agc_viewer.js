...
class AgcViewer extends HTMLElement {
  ...
  _buildSplatMesh(decoded) {
      ...
      new THREE.ShaderMaterial({
        ...
        vertexShader: `
          ...
          void main() {
            ...
            mat3 V = mat3(
              vec3(cov0.x, cov0.y, cov0.z),
              vec3(cov0.y, cov1.x, cov1.y),
              vec3(cov0.z, cov1.y, cov1.z)
            );
            mat3 Rt = mat3(
            ...
            float Jx = uFocal.x / pz;
            float Jy = uFocal.y / pz;
            float a = Jx * Jx * M[0][0] + 0.3;
            float b = Jx * Jy * M[0][1];
            float c = Jy * Jy * M[1][1] + 0.3;

            float mid = 0.5 * (a + c);
            float disc = sqrt(max(0.0, 0.25 * (a - c) * (a - c) + b * b));
            float l1 = clamp(mid + disc, 0.1, 250.0);
            float l2 = clamp(mid - disc, 0.1, 250.0);
            float theta = 0.5 * atan(2.0 * b, a - c);

            float cosT = cos(theta);
            float sinT = sin(theta);
            vec2 majorAxis = sqrt(l1) * vec2(cosT, sinT);
            vec2 minorAxis = sqrt(l2) * vec2(-sinT, cosT);

            vec2 pixelOffset = 2.0 * (coord.x * majorAxis + coord.y * minorAxis);
            vec4 pClip = projectionMatrix * camPos;
            pClip.xy += pixelOffset * (2.0 / uViewport) * pClip.w;
            gl_Position = pClip;
          }
        `,
        fragmentShader: `
          ...
          void main() {
            float d2 = dot(vCoord, vCoord);
            if (d2 > 1.0) discard;
            float alpha = clamp(vColor.a * exp(-2.0 * d2) * 1.4, 0.0, 0.99);
    ...
    if (this.getAttribute('up') === 'z') {
      ...
      const radius = Math.min(extent * 0.55, 10.0);
