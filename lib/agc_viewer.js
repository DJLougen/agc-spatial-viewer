    ...
    const instancedMesh = new THREE.InstancedMesh(
      ...
      new THREE.ShaderMaterial({
        ...
        vertexShader: `
          ...
          varying vec4 vColor;
          varying vec3 vConic;
          varying vec2 vPixelOffset;

          void main() {
            vColor = color;
            vec4 camPos = modelViewMatrix * vec4(instanceMatrix[3].xyz, 1.0);
            if (camPos.z >= -0.05) {
            ...
            mat3 V = mat3(
              cov0.x, cov0.y, cov0.z,
              cov0.y, cov1.x, cov1.y,
              cov0.z, cov1.y, cov1.z
            );
            mat3 Rt = mat3(
              R[0][0], R[1][0], R[2][0],
              R[0][1], R[1][1], R[2][1],
            ...
            float pz = -camPos.z;
            float pz2 = pz * pz;
            float J00 = uFocal.x / pz;
            float J02 = (uFocal.x * camPos.x) / pz2;
            float J11 = uFocal.y / pz;
            float J12 = (uFocal.y * camPos.y) / pz2;

            float c00 = J00 * (M[0][0] * J00 + M[0][2] * J02) + J02 * (M[2][0] * J00 + M[2][2] * J02) + 0.3;
            float c01 = J00 * (M[0][1] * J11 + M[0][2] * J12) + J02 * (M[2][1] * J11 + M[2][2] * J12);
            float c11 = J11 * (M[1][1] * J11 + M[1][2] * J12) + J12 * (M[2][1] * J11 + M[2][2] * J12) + 0.3;
            ...
            vec3 conic = vec3(c11 / det, -c01 / det, c00 / det);
            float mid = 0.5 * (c00 + c11);
            float disc = max(0.0, mid * mid - det);
            float maxLambda = mid + sqrt(disc);
            float radius = clamp(ceil(3.0 * sqrt(max(0.1, maxLambda))), 1.0, 512.0);

            vec2 pixelOffset = (uv - vec2(0.5)) * 2.0 * radius;
            vec4 clipPos = projectionMatrix * camPos;
            clipPos.xy += pixelOffset * (2.0 / uViewport) * clipPos.w;
            gl_Position = clipPos;
            vConic = conic;
        ...
        fragmentShader: `
          ...
          void main() {
            float power = -0.5 * (vConic.x * vPixelOffset.x * vPixelOffset.x +
                                  2.0 * vConic.y * vPixelOffset.x * vPixelOffset.y +
                                  vConic.z * vPixelOffset.y * vPixelOffset.y);
